...
type Schema struct {
	...
	// effectively zero.
	//
	// If AsSingle is true, MaxItems _must_ be 1 and the schema item is then
	// presented to Terraform Core as a single value rather than a list or
	// set, thus making it appear as a single value for references in
	// interpolation. However, this behavior is only for Terraform v0.12 and
	// later; when a provider is used with v0.10 or v0.11 the value will
	// appear instead as a single-element list, for backward-compatibility.
	MaxItems int
	MinItems int
	AsSingle bool
